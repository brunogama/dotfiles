...
grep -q detect-secrets .pre-commit-config.yaml && echo "✓ detect-secrets"
grep -q checkmake .pre-commit-config.yaml && echo "✓ checkmake"
grep -q flake8 .pre-commit-config.yaml && echo "✓ flake8"
...
git commit -m "test" && echo "FAIL: Secret not detected" || echo "✓ Secret detected"
rm test-secret.txt
