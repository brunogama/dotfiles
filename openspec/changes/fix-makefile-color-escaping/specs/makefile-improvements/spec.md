...
# Output: \033[0;32m✓ Success\033[0m (literal escape codes)
```
...
- **THEN** "✓ Installation complete!" SHALL be green
- **AND** "✓ Backup created" SHALL be green
- **AND** "✓ Symlinks created" SHALL be green
...
# Expected: Green ✓ messages for success
...
make check-prezto
# Expected: Yellow ⚠ warning messages
```
...
make help 2>&1 | grep -q '\\033' && echo "❌ FAIL: Literal escapes" || echo "✅ PASS: No literal escapes"

# Test 2: Colors actually present (check for ANSI codes in raw output)
make help 2>&1 | od -An -tx1 | grep -q '1b' && echo "✅ PASS: ANSI codes present" || echo "❌ FAIL: No colors"

# Test 3: All targets work
for target in help test check-prezto; do
  echo "Testing: make $target"
  make $target >/dev/null 2>&1 && echo "  ✅ PASS" || echo "  ❌ FAIL"
done
