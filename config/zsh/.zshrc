...
# ============================================================================
# Environment Configuration (Work vs Personal)
# ============================================================================
# Set DOTFILES_ENV to choose your environment:
#   export DOTFILES_ENV=work      # Load work-config.zsh (shows "WORK" in prompt)
#   export DOTFILES_ENV=personal  # Load personal-config.zsh (shows "HOME" in prompt)
# Default: personal if not set
#
# To switch environments, add to ~/.zshenv or ~/.zprofile:
#   export DOTFILES_ENV=work

if [[ "$DOTFILES_ENV" == "work" ]]; then
    # Load work environment configuration
    if [[ -f ~/.config/zsh/work-config.zsh ]]; then
        source ~/.config/zsh/work-config.zsh
    fi
else
    # Load personal environment configuration (default)
    if [[ -f ~/.config/zsh/personal-config.zsh ]]; then
...
# Directory navigation (zoxide must be initialized)
eval "$(zoxide init zsh)"
# Now use 'z' instead of 'cd' for smart jumping

# Git (use delta for diffs)
export GIT_PAGER='delta'

# Help functions
alias old-find='/usr/bin/find'  # Escape hatch for compatibility
