...
for type in "${SKIP_TYPES[@]}"; do
    if [ "$COMMIT_TYPE" = "$type" ]; then
        echo -e "${GREEN}✓${NC} Skipping CHANGELOG validation for $COMMIT_TYPE commit"
        exit 0
    fi
done
...
if [ "$requires_changelog" = false ]; then
    echo -e "${GREEN}✓${NC} CHANGELOG validation skipped for commit type: $COMMIT_TYPE"
    exit 0
fi
...
if [ "$changelog_modified" = false ]; then
    echo -e "${RED}✗ ERROR: CHANGELOG.md not updated for user-facing change${NC}"
    echo ""
...
# Success!
echo -e "${GREEN}✓${NC} CHANGELOG.md updated for $COMMIT_TYPE commit"
exit 0
