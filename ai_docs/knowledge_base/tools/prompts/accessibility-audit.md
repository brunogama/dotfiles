...
if [ -n "$CHANGED_FILES" ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "[NO] Accessibility tests failed. Please fix issues before committing."
        exit 1
    fi
fi
```
...
Focus on creating inclusive experiences that work for all users, regardless of their abilities or assistive
technologies.
