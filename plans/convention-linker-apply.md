...
1. Change project-root detection to recognize the convention layout.
2. Build a deterministic plan from common, platform, and hostname trees, with later overlay sources winning and equal-precedence conflicts failing before mutation.
3. Discover executable regular files immediately under `bin/<domain>/`; reject duplicate command names before any filesystem mutation.
4. Preflight parent paths and target collisions. Correct links are no-ops; conflicts require explicit backup or replacement confirmation.
5. Persist a versioned ownership ledger with a stable repository identity and exactly the successfully applied entries. If a filesystem failure occurs after preflight, record completed operations atomically, report partial completion, and exit non-zero.
