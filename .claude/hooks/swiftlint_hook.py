        ...
        if result.returncode == 0:
            if "Applied 0 intelligent fixes" in output:
                if violations_info:
                    ...
                    return False, f"⚠️ SwiftLint Proxy: {Path(file_path).name} (violations found but no fixes applied){violations_info}{remaining_violations_info}"
                else:
                ...
                if match:
                    ...
                    if has_remaining_violations:
                        return False, f"⚠️ SwiftLint Proxy: {Path(file_path).name} ({fixes} fixes applied, violations remain){violations_info}{remaining_violations_info}"
                    else:
                ...
                if match:
                    ...
                    if has_remaining_violations:
                        return False, f"⚠️ SwiftLint Proxy: {Path(file_path).name} ({total_fixes} total fixes applied, violations remain){violations_info}{remaining_violations_info}"
                    else:
            ...
            if has_remaining_violations:
                return False, f"⚠️ SwiftLint Proxy: {Path(file_path).name} (violations remain){violations_info}{remaining_violations_info}"
            else:
            ...
            return False, f"⚠️ SwiftLint Proxy: {error_msg}{violations_info}{remaining_violations_info}"
